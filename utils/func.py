...
async def classify_media_streams(file_path: str) -> str:
    """Reliably classifies a local file as 'video', 'audio', or 'document'
    by inspecting its actual streams with ffprobe (not filename/extension).

    - video stream present            -> 'video'
    - no video, audio stream present  -> 'audio'
    - neither video nor audio         -> 'document'

    Falls back to 'video' only if ffprobe itself is unavailable/fails,
    preserving prior behavior for that edge case (extension-based guess
    was already the fallback before this change).
    """
    try:
        ...
        stream_types = stdout.decode(errors="ignore").split()
        ...
        logger.error(f"Error classifying media streams for {file_path}: {e}")
        return "video"


async def get_audio_codec_extension(file_path: str) -> str:
    """Returns a filename extension matching the actual audio codec in the
    file (e.g. '.mp3' for mp3, '.m4a' for aac), so a file misnamed '.mp4'
    is renamed to something Telegram clients render correctly as audio.
    Defaults to '.mp3' if the codec can't be determined."""
    codec_to_ext = {
        "mp3": ".mp3",
        "aac": ".m4a",
        "opus": ".opus",
    ...
    try:
        ...
        stdout, _ = await proc.communicate()
        codec_name = stdout.decode(errors="ignore").strip()
        return codec_to_ext.get(codec_name, ".mp3")
    except Exception as e:
        logger.error(f"Error detecting audio codec for {file_path}: {e}")
        return ".mp3"


async def ensure_correct_audio_extension(file_path: str) -> str:
    """Renames the file (if needed) to match its actual audio codec, so
    send_audio() doesn't hand Telegram a file named '.mp4' that actually
    contains MP3/AAC/etc audio data. Returns the (possibly new) path."""
    correct_ext = await get_audio_codec_extension(file_path)
