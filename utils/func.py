...
async def local_file_has_video_stream(file_path: str) -> bool:
    """Reliably checks a file already on disk for an actual video stream,
    using ffprobe on the full local file (no download-range limitation)."""
    try:
        ...
        logger.error(f"Error probing local file for video stream: {e}")
        return True
