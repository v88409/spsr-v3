...
from utils.func import get_user_data, screenshot, thumbnail, get_video_metadata, has_video_stream
    ...
    try:
        if m.video:
            if await has_video_stream(c, m):
                ...
                await c.send_audio(tcid, m.video.file_id, caption=ft, duration=m.video.duration, file_name=m.video.file_name or None, reply_to_message_id=rtmid)
        elif m.video_note:
            ...
            is_video_ext = doc_name.lower().endswith(video_extensions)
            ...
            if is_audio_mime or is_audio_ext:
                ...
                if await has_video_stream(c, m):
                    await c.send_video(tcid, m.document.file_id, caption=ft, file_name=doc_name or None, reply_to_message_id=rtmid)
                else:
                    await c.send_audio(tcid, m.document.file_id, caption=ft, file_name=doc_name or None, reply_to_message_id=rtmid)
            else:
    ...
    try:
        ...
        if m.media:
            ...
            try:
                ...
                looks_like_video = m.video or (m.document and file_ext in video_extensions)
                is_actual_video = False
                mtd = None
                if looks_like_video:
                    mtd = await get_video_metadata(f)
                    is_actual_video = not (mtd['width'] <= 1 and mtd['height'] <= 1 and mtd['duration'] <= 1)

                if looks_like_video and is_actual_video:
                    dur, h, w = mtd['duration'], mtd['width'], mtd['height']
                    th = await screenshot(f, dur, d)
                    ...
                    await c.send_sticker(tcid, m.sticker.file_id, reply_to_message_id=rtmid)
                elif m.audio or (m.document and file_ext in audio_extensions) or (looks_like_video and not is_actual_video):
