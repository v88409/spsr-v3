...
from utils.func import get_user_data, screenshot, thumbnail, get_video_metadata, classify_media_streams, ensure_correct_audio_extension
...
async def upd_dlg(c):
    try:
        async for _ in c.get_dialogs(limit=300): pass
        return True
    except Exception as e:
...
async def get_msg(c, u, i, d, lt):
    ...
    Returns:
      ...
      - the string "EMPTY" when Telegram positively confirms the message
        is empty/deleted/invalid (safe to count as Skipped)
      - None when the fetch failed for any other/unknown reason
        (must be counted as Failed, not Skipped)
    """
    def _is_confirmed_empty(exc: Exception) -> bool:
        msg = str(exc).upper()
        return (
            'MESSAGE_ID_INVALID' in msg
            or 'MESSAGE_EMPTY' in msg
            or 'MESSAGEIDINVALID' in msg
        )
        ...
        if lt == 'public':
            try:
                if str(i).lower().endswith('bot'):
                    ...
                    is_empty = getattr(xm, "empty", False)
                    emp[i] = not is_empty
                    if not is_empty:
                ...
                if emp[i]:
                    ...
                    if is_empty:
                        ...
                        try:
                            ...
                            if _is_confirmed_empty(e):
                                return "EMPTY"
                            print(f'Error fetching public message (fallback): {e}')
                            return None
            ...
            if u:
                try:
                    async for _ in u.get_dialogs(limit=300): pass
                    ...
                    try:
                        ...
                        if result and not getattr(result, "empty", False):
                            return result
                        if result and getattr(result, "empty", False):
                    ...
                    # Force-resolve the peer directly via get_chat before giving up.
                    try:
                        await u.get_chat(chat_id_100)
                        result = await u.get_messages(chat_id_100, d)
                        if result and not getattr(result, "empty", False):
                            return result
                        if result and getattr(result, "empty", False):
                    ...
                    return "EMPTY" if confirmed_empty else None
...
async def prog(c, t, C, h, m, st, label="Processing"):
    global P
    p = c / t * 100
    ...
    if m not in P or P[m] != step or p >= 100:
        ...
        icon = "⬇️" if label == "Downloading" else "⬆️" if label == "Uploading" else "⚙️"
...
async def send_direct(c, m, tcid, ft=None, rtmid=None):
    try:
        if m.video:
            ...
            doc_name = m.document.file_name or ''
            is_audio_mime = bool(m.document.mime_type) and m.document.mime_type.startswith('audio/')
            is_audio_ext = doc_name.lower().endswith(audio_extensions)
            if is_audio_mime or is_audio_ext:
                await c.send_audio(tcid, m.document.file_id, caption=ft, file_name=doc_name or None, reply_to_message_id=rtmid)
            else:
                await c.send_document(tcid, m.document.file_id, caption=ft, file_name=doc_name, reply_to_message_id=rtmid)
        else:
            return False
        return True
    except Exception as e:
...
async def process_msg(c, u, m, d, lt, uid, i, force_audio=False):
    try:
        ...
        if m.media:
            ...
            try:
                ...
                looks_like_media = m.video or (m.document and (file_ext in video_extensions or file_ext in audio_extensions))
                media_kind = 'audio' if force_audio else None
                mtd = None
                th = None

                if looks_like_media and not force_audio:
                    media_kind = await classify_media_streams(f)
                    if media_kind == 'video':
                        mtd = await get_video_metadata(f)
                ...
                if media_kind == 'video':
                    ...
                    await c.send_sticker(tcid, m.sticker.file_id, reply_to_message_id=rtmid)
                elif m.audio or (m.document and file_ext in audio_extensions) or media_kind == 'audio':
                    if media_kind == 'audio':
                        f = await ensure_correct_audio_extension(f)
...
@X.on_message(filters.command(['batch', 'single', 'fixaudio']))
async def process_cmd(c, m):
    ...
    step = 'start_single' if cmd == 'single' else 'start'
    Z[uid] = {'step': step, 'mode': 'fixaudio' if cmd == 'fixaudio' else 'normal'}
    await pro.edit(f'Send {"start link..." if cmd != "single" else "link you to process"}.')
...
@X.on_message(filters.text & filters.private & ~login_in_progress & ~filters.command([
    ...
    'pay', 'redeem', 'gencode', 'single', 'fixaudio', 'generate', 'keyinfo', 'encrypt', 'decrypt', 'keys', 'setbot', 'rembot']))
async def text_handler(c, m):
    ...
    if s == 'start':
        ...
        try:
            ...
            if msg == "EMPTY":
                await pt.edit('⏭️ Message is empty or deleted.')
            elif msg:
                res = await process_msg(ubot, uc, msg, str(m.chat.id), lt, uid, i, force_audio=(Z[uid].get('mode') == 'fixaudio'))
        ...
        try:
                ...
                try:
                    if confirmed_empty:
                        ...
                        if 'Done' in res or 'Copied' in res or 'Sent' in res:
                            ...
                            failed_reasons.append(f'mid {mid}: {res}')
                    else:
                        failed_reasons.append(f'mid {mid}: {last_err}')
                except Exception as e:
                    failed_reasons.append(f'mid {mid}: {str(e)[:50]}')
            ...
            if j+1 == n:
                ...
                if failed_reasons:
                    shown = failed_reasons[:10]
                    summary += '\n\n' + '\n'.join(shown)
                    if len(failed_reasons) > 10:
                        summary += f'\n+{len(failed_reasons) - 10} more'
