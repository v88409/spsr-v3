...
from utils.func import a1, a2, a3, a4, a5, a7, a8, a9, a10, a11, users_collection, get_branding_settings
...
WELCOME_TEXT = (
    "Hi 👋 Welcome to Team SPY V3!\n\n"
    "✳️ I save posts from channels/groups where forwarding is off, and download "
    "videos/audio from YouTube, Instagram & other platforms.\n\n"
    "📥 **How to use:**\n"
    "• Public channel/post → just send the link\n"
    "• Private channel → use /login first, then send the link\n"
    "• Multiple posts at once → use /batch after logging in\n\n"
    "💎 Free users get limited downloads — send /plan for premium limits & pricing.\n\n"
...
async def start_handler(client, message):
    ...
    jl = settings["join_link"]
    ac = settings["admin_contact"]

    rows = []
    if jl["enabled"] and jl["value"]:
        rows.append([IK("Join Channel", url=jl["value"])])
    if ac["enabled"] and ac["value"]:
        rows.append([IK("Get Premium", url=ac["value"])])
    else:
        rows.append([IK("Get Premium", callback_data="contact_not_set")])
    rows.append([IK("📖 View Commands", callback_data="help_view_0")])
