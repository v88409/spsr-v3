...
from utils.func import get_all_user_ids, get_all_premium_user_ids, BOT_START_TIME
...
async def stats_handler(event):
    ...
    premium_entries = await get_all_premium_user_ids()

    mem = psutil.virtual_memory()
    cpu_percent = psutil.cpu_percent(interval=0.5)
    uptime = _format_uptime(datetime.now() - BOT_START_TIME)

    text = (
        "📊 **Bot Statistics**\n\n"
        f"👥 Total Users: {len(all_ids)}\n"
        f"💎 Premium Users: {len(premium_entries)}\n"
        f"🖥️ RAM: {mem.used // (1024*1024)}MB / {mem.total // (1024*1024)}MB ({mem.percent}%)\n"
        f"⚙️ CPU: {cpu_percent}%\n"
        f"⏱️ Uptime: {uptime}"
    )
